...
namespace NoSqlQuery
{
	...
	public:
		...
		Query<T>& selectChildren(const std::string& arg);
		...
		DbElement<T> ReturnValue(const std::string& arg);
	...
	template <typename T>
	DbElement<T> Query<T>::ReturnValue(const std::string& arg)
	{
		DbCore<T>::DbStore dbStore_ = db_.dbStore();
		std::cout << "\n\n--------------------------------------------------------";
		std::cout << "\n Querying for the key \"" << arg << "\" in the database to retrieve its value(DbElement)";
		if (db_.contains(arg))
		{
			std::cout << "\n found the key \"" << arg << "\" in the database" << "\n The value for the key is";
			DbElement<T> dbElem = dbStore_[arg];
	...
	Query<T>& Query<T>::showChildren(const std::string& arg)
	{
		...
		if (db_.contains(arg))
		{
			...
			Children children = dbStore_[arg].children();  // note Keys& - we don't want copy of children
			if (children.size() > 0)
			{
				std::cout << "\n    child keys: ";
	...
	Query<T>& Query<T>::selectName(const std::string& arg)
	{
		...
		keys_.clear();
		std::cout << "\n\n--------------------------------------------------------";
		std::cout << "\n  Querying on metadata for the file \"" << arg << "\"";
		std::regex regx(arg);
		for (Key k : keys)
		{
	...
	//prints the keys that match with the regular expression pattern in the meta data's children
	template <typename T>
	Query<T>& Query<T>::selectChildren(const std::string& arg)
	{
		DbCore<T>::DbStore dbStore_ = db_.dbStore();
		Keys keys = keys_;
		keys_.clear();
		std::cout << "\n\n--------------------------------------------------------";
		std::cout << "\n  Searching  children match for the pattern \"" << arg << "\"";
		std::regex regx(arg);
		for (Key k : keys)
		{
			DbElement<T> dbElem = dbStore_[k];
		...
		if (keys_.size() > 0)
		{
			std::cout << "\n found  children match for the pattern \"" << arg << "\"\n The matched keys are:\n";
			for (Key k : keys_)
				std::cout << k << " ";
		}
		else
	...
	*/
	...
	Query<T>& Query<T>::selectPayloadCategory(const std::string& arg)
	{
		...
		for (Key k : keys)
		{
			DbElement<T> dbElem = dbStore_[k];
			PayLoad dbElemPayload = dbElem.payLoad();
			Categories categories = dbElemPayload.categories();
