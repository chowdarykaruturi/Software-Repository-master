...
const std::string clientStorage = "../../../../ClientStorage";
const std::string serverStorage = "../StagingPath";
...
bool Sender::sendFile(Message msg)
{
  ...
  if (!msg.containsKey("file"))
    return false;
  std::string sendPath = msg.sendPath();
  if (sendPath != "" && (msg.name() == "checkin" || msg.name() == "viewfile"))
  {
	  fileSpec = sendPath + "/" + msg.file();
  }

  else if (sendPath == "" && msg.name() == "checkout") {
	  fileSpec = serverStorage + "/" + msg.file();
  }
 std::cout<<"\n Sending file from "<< Path::getFullFileSpec(fileSpec);
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	std::string receivePath = msg.receivePath();
	std::string fileSpec;
	if (receivePath == "" && msg.name() == "checkin") {
		fileSpec = serverStorage + "/" + fileName;
	}
	else if (msg.name() == "checkout" || msg.name() == "viewfile") {
		fileSpec = receivePath + "/" + fileName;
	}
	
	std::cout << "\nReceiving file to "<< Path::getFullFileSpec(fileSpec);
